...
Nixbot is a rewrite of
[buildbot-nix](https://github.com/nix-community/buildbot-nix), its spiritual
ancestor, and no longer runs on top of Buildbot. The old master/worker pair is
...
**NixOS module.** Import `nixosModules.nixbot`; options live under
`services.nixbot`. Your existing `services.buildbot-nix.master.*` config keeps
working: options rename to `services.nixbot.*` automatically and print
...
**Per-repository config.** The file is now called `nixbot.toml`; the legacy
`buildbot-nix.toml` name keeps working with unchanged contents.
...
- `%(secret:NAME)s` reads systemd credentials of the `nixbot` unit, so move
...
- GitLab support (`services.nixbot.gitlab`): token-based, with per-repository
  webhooks and commit statuses. See docs/GITLAB.md.
