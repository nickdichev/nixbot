...
Ported from nixbot.models with all buildbot couplings removed:
no `Interpolate.to_buildbot`, no buildbot step construction, no
master/worker options. nixbot is a single service, so options that
...
class Config(BaseModel):
    """Top-level configuration for the nixbot service."""
    ...
    state_dir: Path = Path("/var/lib/nixbot")
    ...
    gcroots_dir: Path = Path("/nix/var/nix/gcroots/per-user/nixbot")
