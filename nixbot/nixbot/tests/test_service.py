...
async def test_pr_close_discards_queued_changes(
    postgres_dsn: str, tmp_path: Path
) -> None:
    """A queued change event for a closed PR must not build it later."""

    service, _app = await build_service(make_config(postgres_dsn, tmp_path / "state"))
    pool = service.pool
    try:
...
async def test_cancel_not_running_posts_forge_status(
    postgres_dsn: str, tmp_path: Path
) -> None:
    service, _app = await build_service(make_config(postgres_dsn, tmp_path / "state"))
    pool = service.pool
...
async def test_cancel_not_running_settles_attribute_rows(
    postgres_dsn: str, tmp_path: Path
) -> None:
