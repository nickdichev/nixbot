...
async def test_enqueue_dedupes_pending(postgres_dsn: str) -> None:
...
async def test_claim_serializes_per_key(postgres_dsn: str) -> None:
    async with db_pool(postgres_dsn) as pool:
        queue = WorkQueue(pool)
...
async def test_settle_interrupted_requeues(postgres_dsn: str) -> None:
    async with db_pool(postgres_dsn) as pool:
        queue = WorkQueue(pool)
