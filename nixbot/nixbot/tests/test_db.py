...
async def test_migrations_idempotent(postgres_dsn: str) -> None:
...
async def test_huge_attr_name_does_not_break_notify_trigger(postgres_dsn: str) -> None:
...
async def test_schema_constraints(postgres_dsn: str) -> None:
...
async def test_failed_status_store_component(postgres_dsn: str) -> None:
...
async def test_failed_build_cache_component(postgres_dsn: str) -> None:
    async with db_pool(postgres_dsn) as pool:
...
async def test_cancelled_build_not_reused(postgres_dsn: str) -> None:
...
async def test_reuse_backfills_pr_fields(postgres_dsn: str) -> None:
...
async def test_reuse_by_other_pr_clears_both_identity_fields(postgres_dsn: str) -> None:
...
async def test_branch_push_takes_over_reused_pr_build(postgres_dsn: str) -> None:
...
async def test_pr_does_not_capture_branch_push_build(postgres_dsn: str) -> None:
...
async def test_complete_attribute_preserves_eval_outputs(postgres_dsn: str) -> None:
...
async def test_complete_attribute_marks_substituted_as_cached(
    postgres_dsn: str,
) -> None:
    ...
    async with db_pool(postgres_dsn) as pool:
        ...
        async def cached(attr: str) -> bool:
            return await pool.fetchval(
                "SELECT cached FROM build_attributes WHERE build_id = $1 AND attr = $2",
                build.id,
                attr,
            )
...
async def test_complete_attribute_replaces_log_row(postgres_dsn: str) -> None:
...
async def test_mark_attribute_building_does_not_resurrect_cancelled_rows(
    postgres_dsn: str,
) -> None:
...
async def test_mark_attribute_building_sets_status_and_started_at(
    postgres_dsn: str,
) -> None:
    async with db_pool(postgres_dsn) as pool:
...
async def test_complete_attribute_if_unfinished_skips_terminal_rows(
    postgres_dsn: str,
) -> None:
...
async def test_failed_build_cache_scoped_per_project(postgres_dsn: str) -> None:
